...
- Voice in (STT): **faster-whisper**, local/offline (Phase 1). **Defaults to English (`WHISPER_LANGUAGE=en`)** for speed + accuracy — with `en` it skips transliteration. The **Hinglish path is kept but dormant**: `WHISPER_LANGUAGE=hi`/blank re-enables auto-detect + **romanise Hindi to Latin** (real words, never Devanagari/Urdu) — a Phase-2 differentiator, not deleted. **Run `large-v3` on `cuda`/`float16` on the GPU desktop** (~2-3s for a 12s clip vs ~20s on `small`/CPU). Tunable via `.env`: `WHISPER_MODEL`, `WHISPER_DEVICE` (`cpu`/`cuda`), `WHISPER_COMPUTE`, `WHISPER_LANGUAGE`. The CUDA→CPU fallback is safe but **must be logged loudly** — it silently ran on CPU before, the real cause of the lag. Cloud option for Phase 2: Deepgram (streaming)
- Voice out (TTS): **edge-tts** neural voices (Microsoft, free / no key), served from the backend at `POST /speak` as MP3 the frontend plays — browser-independent. Replaced browser SpeechSynthesis (robotic in some Chromium builds, silent in others). Voice set via `ZENITH_TTS_VOICE` (default `en-IN-NeerjaNeural` / `en-IN-PrabhatNeural`). **Kokoro** (hexgrad/Kokoro-82M) is now **implemented as a local/offline engine**, selected via `ZENITH_TTS_ENGINE=kokoro` (edge stays the fallback). English voices (default `af_heart`), runs on CPU at ~0.5× realtime (~1.4s for a short reply), returns WAV; needs the **Python 3.11** backend venv (spacy/blis have no 3.14 wheels). Kokoro's Hindi voices are weaker → English-only for now. Piper still possible. (Reply-lag: edge = cloud round-trip, Kokoro = local synth — pre-fetch/stream later either way.)
...
- **TTS (v1.6 — SHIPPED):** **Kokoro is now the local/offline engine** (`ZENITH_TTS_ENGINE=kokoro`, default voice `af_heart`, English; edge-tts kept as the one-flag fallback). `synthesize()` dispatches by engine and returns `(bytes, media_type)` — edge→MP3, Kokoro→WAV — so `/speak` + the frontend serve either unchanged. Needs the **Python 3.11** backend venv (spacy/blis lack 3.14 wheels); the old 3.14 venv is parked as `backend/.venv314-bak`. CPU ~0.5× realtime (~1.4s short reply, one-time ~11s model load on boot); GPU optional via `ZENITH_KOKORO_DEVICE=cuda`. `/health` reports the active engine/voice. Kokoro Hindi is weak → English-only for now. Reply-lag still exists — pre-fetch/stream is **TODO §4**.

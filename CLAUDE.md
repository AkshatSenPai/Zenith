...
- **Skins / themes (designed 2026-06-22, build next):** the palette is being tokenized into **CSS variables** keyed by `data-skin` on `<html>` (Tailwind colors → `rgb(var(--..)/<alpha>)` so utility classes auto-theme; the orb reads colors/bloom/particle-count from the same vars). Three skins: **Arc** (the cyan palette above — default, unchanged), **Ghost** (mono silver-white on pure black; glow/bloom killed, square corners, calmer motion, **centered-focus layout** that hides the dashboard columns; one muted amber kept for alerts), **Amethyst** (violet `#B26BFF`; **rounded-glass** cards + **bento layout**, orb as a hero tile — owner-approved mockup). Picked from the **Settings** view; switch uses an emil **blur-mask crossfade**. A skin = colors + treatment knobs (`--glow-strength`, `--panel-tint`, `--notch`/`--radius`, `--motion-scale`, `--bloom`, `--particle-count`). Spec+plan in `docs/superpowers/{specs,plans}/2026-06-22-zenith-skins*.md`; built with the **Impeccable** plugin + taste / minimalist / emil.
...
- **Skins (v1.8 — DESIGNED, build next):** tokenize the palette into CSS vars keyed by `data-skin`; three skins — **Arc** (cyan default, unchanged) / **Ghost** (mono, centered-focus layout) / **Amethyst** (violet, rounded-glass **bento** layout). Picked from Settings; switch = emil blur-mask crossfade. Spec+plan in `docs/superpowers/{specs,plans}/2026-06-22-zenith-skins*.md`; built with the **Impeccable** plugin + taste / minimalist / emil. (See UI STYLE.)

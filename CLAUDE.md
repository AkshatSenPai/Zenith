...
- Voice out (TTS): **edge-tts** neural voices (Microsoft, free / no key), served from the backend at `POST /speak` as MP3 the frontend plays — browser-independent. Replaced browser SpeechSynthesis (robotic in some Chromium builds, silent in others). Voice set via `ZENITH_TTS_VOICE` (default `en-IN-NeerjaNeural` / `en-IN-PrabhatNeural`). **Kokoro** (hexgrad/kokoro) is the future **local/offline** option — verify Hindi support before switching; Piper also possible. (Reply-lag is edge-tts's round-trip — pre-fetch/stream later.)
...
- **Voice round-trip:** mic (MediaRecorder, hold space) → `POST /transcribe` (faster-whisper, English default) → `POST /chat` (Claude + last-20 history + rate limit + Hinglish prompt + tools + confirm gate) → reply text → `POST /speak` (edge-tts → MP3) → frontend plays the audio. Reply text is rendered as markdown (emojis stripped) before display and before TTS.
- **Delivery:**
...
- **TTS option:** edge-tts stays default (English voices Neerja/Prabhat). **Kokoro** logged as a future offline-TTS option (verify Hindi first). Reply-lag = edge-tts round-trip (separate from STT); pre-fetch/stream later.
...
Stack: Next.js + Tauri desktop shell + Python FastAPI + Claude Sonnet 4.6 (tool use) + faster-whisper (local STT, **English default**) + edge-tts neural TTS (backend `/speak` → MP3) + react-three-fiber (particle-sphere orb) + PostgreSQL.
